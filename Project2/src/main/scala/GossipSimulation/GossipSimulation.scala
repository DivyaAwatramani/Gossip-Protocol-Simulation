...
class MainActor(node: Int,topology :String ,protocol: String) extends Actor {
  ...
  def line() {
    ...
    for(i<- 1 until nodes-1)		//set the neighbours for the remaining actors:each node will have two neighbours 
    {
      tempNeighborList= new ArrayBuffer[ActorRef]()
  ...
  def ThreeDPoint() : (Array[Int], Array[Int], Array[Int], Int) = {
    val c:Int = Math.ceil(Math.cbrt(nodes)).toInt
  ...
  if(topology == "3D" || topology == "Imp3D") {
    var t:Int = Math.ceil(Math.cbrt(node)).toInt
    nodes = t*t*t
  }
  ...
  def receive = {
    ...
    case Completed =>
      if(protocol=="PushSum") {			// system will converge here as one of the node has reached the non changing s/w ratio
        ...
        println("Time taken in PUSH SUM distribution using "+ topology+ " is "+ (FinishTime - StartTime) + " milliseconds")
      ...
      if(protocol=="Gossip") {			
        ...
        if(TerminatedActorCount== nodes){
          FinishTime= System.currentTimeMillis()
          //println("Gossip completed at time: "+ FinishTime)
      ...
      case GossipReached =>
        ...
        if(GossipReceiverCount == nodes) {		// if all the nodes have received the gossip atleast once then also system converges
          FinishTime= System.currentTimeMillis()
          //println("Finish Time: " + FinishTime)
          println("All the nodes have received the gossip atleast in " + (FinishTime - StartTime) + " milliseconds once hence shutting down the system---")
          context.system.shutdown()
        }
...
class GossipActor(master: ActorRef) extends Actor {
  ...
  def receive = {
       ...
       // if(Wvalue==1)
        ...
        if (NoChange<3) {
          ...
          //for(i<-0 to neighbourList.length-1)
            ...
            println("Sum value --> " + Sindex + " \nWeight value --> " + Wvalue)
            println("S/W Ratio --> " + Sindex/Wvalue)
            master ! Completed
        }
      }

    case RemoveActor(callingActor) =>			//REMOVE THE TERMINATED ACTOR FROM THE NEIGHBOUR LIST
    ...
    case StartGossip =>
      if(live==1) {        
        if(gossipCount<60 && neighbourList.length>0) {
          if(gossipCount==0) {					//RECEIVED GOSSIP FIRST TIME
            //println ("Gossip Reached "+ this.self)
          ...
          live=0
          //println("not trasnsmitting gossips anymore")
